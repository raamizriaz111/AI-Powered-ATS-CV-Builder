...
export interface ATSIssue {
  id: string; category: string; severity: 'critical' | 'warning' | 'suggestion';
  message: string; fix: string;
}
export interface ATSCategoryScore { name: string; score: number; maxScore: number; issues: ATSIssue[]; }
export interface ATSResult { total: number; categories: ATSCategoryScore[]; issues: ATSIssue[]; passedChecks: string[]; }
